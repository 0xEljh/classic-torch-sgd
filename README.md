...
With these adjustments, it should be easier to tune and understand the implications of the learning rate and momentum settings in the SGD optimizer.
...
You will find that there are differences in behavior right away.
A "truly equivalent" `ClassicSGD` counterpart would have momentum, $\mu$, scaled by `lr`.

### Installation

You may skip this installation step if you're using `uv`.
In-line dependency declarations were added to the comparison script.
...
uv run compare_optimizers.py # replace `uv run` with `python` if not using `uv`

# Custom configuration
uv run compare_optimizers.py --epochs 60 --lr 0.1 --momentum 0.95 --patience 5

# Headless mode (no plot display, saves to file)
uv run compare_optimizers.py --no-plot --save-plot results.png
```
