...
def train_with_optimizer(
    ...
    for epoch in range(num_epochs):
        ...
        if current_lr < prev_lr:
            ...
            print(f"  [{name}] Epoch {epoch}: LR reduced {prev_lr:.6f} -> {current_lr:.6f}")
        prev_lr = current_lr
...
def plot_comparison(pytorch_history, classic_history, save_path=None):
    ...
    ax1 = axes[0, 0]
    ax1.plot(epochs, pytorch_history['train_loss'], 'b-', label='PyTorch SGD', alpha=0.8)
...
def main():
    ...
    parser.add_argument('--save-plot', type=str, default='comparison_results.png',
    ...
    print(f"  Scheduler:       ReduceLROnPlateau (patience={args.patience}, factor={args.factor})")
    ...
    pytorch_history = train_with_optimizer(
        ...
        pytorch_optimizer, pytorch_scheduler,
        args.epochs, device, "PyTorch SGD"
    )
    ...
    classic_history = train_with_optimizer(
        classic_model, train_loader, val_loader,
        classic_optimizer, classic_scheduler,
        args.epochs, device, "ClassicSGD"
    )
